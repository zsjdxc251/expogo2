...
const DEFAULT_SETTINGS = { autoSubmit: false, fontKey: 'default' };
...
export function AppProvider({ children }) {
  ...
  const buildErrorReview = useCallback(() => {
    ...
    const unique = allWrong.filter((q) => {
      const k = q.stem
        ? `${q.op || ''}-${q.stem}-${q.answer}`
        : `${q.left}-${q.op}-${q.right}-${q.missingPos}`;
  ...
  const finishQuiz = useCallback(async (data) => {
    ...
    const correct = data.correct ?? questions.filter((q, i) => isCorrect(q, answers[i])).length;
    const total = data.total ?? questions.length;
    const wrong = data.wrong ?? (total - correct);
    const wrongList = data.wrongList ?? questions
