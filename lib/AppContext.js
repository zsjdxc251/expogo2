...
import { loadDailyTasks, saveDailyTasks, updateTaskProgress, REWARD_PER_TASK } from './dailyTasks';

const DEFAULT_SETTINGS = { autoSubmit: false };
const DEFAULT_BREAK = { usageMinutes: 20, breakMinutes: 5 };
const DEFAULT_REWARD = { perCorrect: 5, perfectBonus: 10 };
...
export function AppProvider({ children }) {
  ...
  const finishQuiz = useCallback(async (data) => {
    ...
    const dailyFirst = !history.some((h) => typeof h.date === 'string' && h.date.startsWith(today));
    const rc = user?.rewardConfig || DEFAULT_REWARD;
    const pointsEarned = calcPoints({ total, correct, elapsed, maxCombo, dailyFirst, perCorrect: rc.perCorrect, perfectBonus: rc.perfectBonus });
    ...
    const taskBonus = (newCompleted - prevCompleted) * REWARD_PER_TASK;
