...
// ── 识字表：从 LEARN_LITERACY 提取字+拼音 ────────────────
// Grouped by learn card for course-by-course practice

const SHIZI_UNITS = [
  { key: 'gaopinzi1', label: '常用汉字（一）', topic: 'gaopinzi',
    chars: [
      { char: '人', pinyin: 'rén' }, { char: '大', pinyin: 'dà' },
      { char: '小', pinyin: 'xiǎo' }, { char: '上', pinyin: 'shàng' },
      { char: '下', pinyin: 'xià' }, { char: '左', pinyin: 'zuǒ' },
      { char: '右', pinyin: 'yòu' }, { char: '中', pinyin: 'zhōng' },
      { char: '天', pinyin: 'tiān' }, { char: '地', pinyin: 'dì' },
    ]},
  { key: 'gaopinzi2', label: '常用汉字（二）', topic: 'gaopinzi',
    chars: [
      { char: '日', pinyin: 'rì' }, { char: '月', pinyin: 'yuè' },
      { char: '水', pinyin: 'shuǐ' }, { char: '火', pinyin: 'huǒ' },
      { char: '山', pinyin: 'shān' }, { char: '石', pinyin: 'shí' },
      { char: '田', pinyin: 'tián' }, { char: '土', pinyin: 'tǔ' },
      { char: '木', pinyin: 'mù' }, { char: '禾', pinyin: 'hé' },
    ]},
  { key: 'gaopinzi3', label: '常用汉字（三）', topic: 'gaopinzi',
    chars: [
      { char: '学', pinyin: 'xué' }, { char: '习', pinyin: 'xí' },
      { char: '字', pinyin: 'zì' }, { char: '写', pinyin: 'xiě' },
      { char: '读', pinyin: 'dú' }, { char: '书', pinyin: 'shū' },
      { char: '笔', pinyin: 'bǐ' }, { char: '纸', pinyin: 'zhǐ' },
      { char: '课', pinyin: 'kè' }, { char: '本', pinyin: 'běn' },
    ]},
  { key: 'pianpang1', label: '偏旁字（水·人·口）', topic: 'pianpang',
    chars: [
      { char: '河', pinyin: 'hé' }, { char: '湖', pinyin: 'hú' },
      { char: '海', pinyin: 'hǎi' }, { char: '洗', pinyin: 'xǐ' },
      { char: '你', pinyin: 'nǐ' }, { char: '他', pinyin: 'tā' },
      { char: '们', pinyin: 'men' }, { char: '住', pinyin: 'zhù' },
      { char: '吃', pinyin: 'chī' }, { char: '喝', pinyin: 'hē' },
      { char: '叫', pinyin: 'jiào' }, { char: '唱', pinyin: 'chàng' },
    ]},
  { key: 'pianpang2', label: '偏旁字（女·木·草）', topic: 'pianpang',
    chars: [
      { char: '妈', pinyin: 'mā' }, { char: '姐', pinyin: 'jiě' },
      { char: '妹', pinyin: 'mèi' }, { char: '好', pinyin: 'hǎo' },
      { char: '树', pinyin: 'shù' }, { char: '林', pinyin: 'lín' },
      { char: '森', pinyin: 'sēn' }, { char: '桥', pinyin: 'qiáo' },
      { char: '花', pinyin: 'huā' }, { char: '草', pinyin: 'cǎo' },
      { char: '苗', pinyin: 'miáo' }, { char: '菜', pinyin: 'cài' },
    ]},
  { key: 'xingjinzi', label: '形近字', topic: 'xingjinzi',
    chars: [
      { char: '入', pinyin: 'rù' }, { char: '太', pinyin: 'tài' },
      { char: '士', pinyin: 'shì' }, { char: '目', pinyin: 'mù' },
      { char: '由', pinyin: 'yóu' }, { char: '己', pinyin: 'jǐ' },
      { char: '已', pinyin: 'yǐ' }, { char: '夫', pinyin: 'fū' },
    ]},
];

export const SHIZI_TABLE = SHIZI_UNITS;
export const ALL_SHIZI_CHARS = SHIZI_UNITS.flatMap((u) => u.chars);

function stripTone(py) {
  return py.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
}

export function getShiziChars(unitKey, unfamiliarList) {
  if (unitKey === 'unfamiliar') {
    return ALL_SHIZI_CHARS.filter((c) => unfamiliarList.includes(c.char));
  }
  const unit = SHIZI_UNITS.find((u) => u.key === unitKey);
  return unit ? unit.chars : [];
}

export function genShiziPinyinQuestions(unitKey, count, unfamiliarList) {
  let pool = getShiziChars(unitKey, unfamiliarList || []);
  const shuffled = [...pool].sort(() => Math.random() - 0.5).slice(0, count);
  const allPy = [...new Set(ALL_SHIZI_CHARS.map((c) => c.pinyin))];
