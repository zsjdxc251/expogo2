...
const MATCHERS = {
  ...
  mulForward:   (s) => s === 'mulForward',
  mulBlank:     (s) => s === 'mulBlank',
  add:          (s) => s === 'add',
  subtract:     (s) => s === 'subtract',
  divide:       (s) => s === 'divide',
  divRem:       (s) => s === 'divRem',
  divReverse:   (s) => s === 'divReverse',
  addTwo:       (s) => s === 'addTwo',
  subtractTwo:  (s) => s === 'subtractTwo',
  mulReverse:   (s) => s === 'mulReverse',
  compare:      (s) => s === 'compare',
  wordProblem:  (s) => s === 'wordProblem',
  pattern:      (s) => s === 'pattern',
...
function subjectToType(subject) {
  ...
  if (subject === 'speed') return 'speed';
  if (subject === 'dictation') return 'dictation';
  if (subject?.startsWith('chn_')) return 'chn';
  return 'math';
}
