...
export function calcPoints({ total, correct, elapsed, maxCombo, dailyFirst }) {
  let pts = correct * 10;
  if (correct === total && total > 0) pts += 50;
  if (total > 0) {
    const avg = elapsed / total;
    if (avg < 5) pts += 30;
    else if (avg < 10) pts += 20;
  }
  if (maxCombo >= 5) pts += 20;
  else if (maxCombo >= 3) pts += 10;
  if (dailyFirst) pts += 20;
  return pts;
}
...
export const ACH_DEFS = [
  ...
  { id: 'allSubjects', name: '全科冠军', desc: '4科目各完成至少1次',   icon: '🏆' },
...
export function checkNewAchievements({ user, history, record, streak, unlocked }) {
  ...
  if (!has('allSubjects')) {
    const subs = new Set(history.map((h) => h.subject));
    subs.add(record.subject);
    if (subs.size >= 4) fresh.push('allSubjects');
  }
