...
function pick(positions) {
  return positions[Math.floor(_rng() * positions.length)];
}
...
function genAddTwo(count, [lo, hi]) {
  ...
  for (let i = 0; i < 200; i++) {
    const a = scale + Math.floor(_rng() * (99 - scale));
    const b = scale + Math.floor(_rng() * (99 - scale));
...
function genSubtractTwo(count, [lo, hi]) {
  ...
  const scale = lo <= 3 ? 10 : lo <= 5 ? 20 : 30;
  for (let i = 0; i < 200; i++) {
    const b = scale + Math.floor(_rng() * (50 - scale));
    const diff = 1 + Math.floor(_rng() * (b - 1));
    const a = b + diff;
...
function genMulReverse(count, [lo, hi]) {
  ...
  for (let a = lo; a <= hi; a++) {
    for (let b = a; b <= hi; b++) {
      const product = a * b;
      const wrong1 = product + Math.floor(_rng() * 3) + 1;
      const wrong2 = product - Math.floor(_rng() * 3) - 1;
      const wrong3 = (a + 1) * b;
...
function genCompare(count, [lo, hi]) {
  ...
  const OPS = ['>', '<', '='];
  for (let i = 0; i < 150; i++) {
    const a1 = lo + Math.floor(_rng() * (hi - lo + 1));
    const b1 = lo + Math.floor(_rng() * (hi - lo + 1));
    const a2 = lo + Math.floor(_rng() * (hi - lo + 1));
    const b2 = lo + Math.floor(_rng() * (hi - lo + 1));
    const leftVal = a1 * b1;
...
function genWordProblem(count, [lo, hi]) {
  ...
  for (let i = 0; i < attempts; i++) {
    const t = WORD_TEMPLATES[Math.floor(_rng() * WORD_TEMPLATES.length)];
    let a, b;
    if (t.genRange) {
      [a, b] = t.genRange(lo, hi);
    } else {
      a = lo + Math.floor(_rng() * (hi - lo + 1));
      b = lo + Math.floor(_rng() * (hi - lo + 1));
    }
...
export function generateQuestions(subject, count, range, seed) {
  const prevRng = _rng;
  if (seed != null) _rng = makeSeededRng(seed);
  try {
