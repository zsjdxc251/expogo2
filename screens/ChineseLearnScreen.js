...
export default function ChineseLearnScreen({ topicKey, onBack, onPractice }) {
        ...
        <Animated.View style={[st.card, { opacity: fadeAnim, borderTopColor: topic.color }]}>
          ...
          {card.examples && card.examples.length > 0 ? (
            <View style={st.exSection}>
              <Text style={st.exTitle}>📝 示例</Text>
              {card.examples.map((ex, i) => (
                <View key={i} style={st.exItem}>
                  <View style={st.exMainRow}>
                    <Text style={[st.exMain, { flex: 1 }]}>{ex.zh}</Text>
                    <SpeakButton text={ex.zh} size="small" language="zh-CN" />
                  </View>
                  <Text style={st.exDesc}>{ex.en}</Text>
                </View>
              ))}
            </View>
          ) : null}
...
const st = StyleSheet.create({
  ...
  exMainRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 2 },
  exMain: { fontSize: 16, fontWeight: '600', color: C.accent },
  exDesc: { fontSize: 14, color: C.textMid },
